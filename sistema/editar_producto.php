...
if (!empty($_POST)) {
  $alert = "";
  if (empty($_POST['codigo']) || empty($_POST['producto']) || empty($_POST['precio']) || empty($_POST['categoria'])) {
    ...
    $query_update = mysqli_query($conexion, "UPDATE producto SET codigo = '$codigo', descripcion = '$producto', proveedor= $proveedor, precio= $precio, categoria= '$categoria' WHERE codproducto = $codproducto");
    if ($query_update) {
      $alert = '<div class="alert alert-primary" role="alert">
              Producto Modificado
            </div>';
...
if (empty($_REQUEST['id'])) {
  ...
  $query_producto = mysqli_query($conexion, "SELECT p.codproducto, p.codigo, p.descripcion, p.precio, p.categoria, pr.codproveedor, pr.proveedor FROM producto p INNER JOIN proveedor pr ON p.proveedor = pr.codproveedor WHERE p.codproducto = $id_producto");
        ...
        <div class="card-body">
          <form action="" method="post">
            ...
            <div class="form-group">
              ...
              <input type="text" placeholder="Ingrese la categoria" class="form-control" name="categoria" id="categoria" value="<?php echo $data_producto['categoria']; ?>">
            </div>
